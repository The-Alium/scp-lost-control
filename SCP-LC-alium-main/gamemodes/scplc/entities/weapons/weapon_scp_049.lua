...
local function translate_zombie_model( ply, rag )
	...
	if team == TEAM_CLASSD or class == CLASSES.CIAGENT then
		return "models/player/zombie_fast.mdl", math.random( 0, 4 )
	elseif team == TEAM_MTF then
		if string.find( rag:GetModel(), "models/player/alski/security" ) then
			return "models/player/zombie_soldier.mdl", math.random( 0, 4 ) * 5 + rag:GetSkin()
		else
			if math.random( 1, 2 ) == 1 then
				return "models/player/zombie_soldier.mdl", math.random( 0, 4 )
			else
				return "models/player/zombie_soldier.mdl", math.random( 0, 4 )
			end
		end
	elseif team == TEAM_SCI then
		return "models/player/zombie_classic.mdl", math.random( 0, 4 )
