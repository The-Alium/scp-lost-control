...
function SWEP:PrimaryAttack()
	...
	self:SetNextPrimaryFire( ct + 1.5 - ( self:GetUpgradeMod( "prim_cd" ) or 0 ) )
...
function SWEP:SecondaryAttack()
	...
	if self:GetUpgradeMod( "shot_mode" ) == 2 then
		...
		self:SetNextSecondaryFire( ct + 3 + ( self:GetUpgradeMod( "shot_cd" ) or 0 ) )
	end
