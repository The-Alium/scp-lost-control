...
concommand.Add("GetPlayerKarma", function (ply, cmd, args, argStr)
   for index, value in player.Iterator() do
      print("Player:"..tostring(value:Nick()).." Karma:"..tostring(value:GetLiveKarma()))
   end
end)
...
KARMA.cv.ratio       = CreateConVar("slc_karma_ratio", "0.001")
KARMA.cv.killpenalty = CreateConVar("slc_karma_kill_penalty", "15")
KARMA.cv.roundheal   = CreateConVar("slc_karma_round_increment", "5")
KARMA.cv.clean  = CreateConVar("slc_karma_clean_bonus", "30")
KARMA.cv.bonus = CreateConVar("slc_karma_kill_bonus", "20")
KARMA.cv.tratio = CreateConVar("slc_karma_traitordmg_ratio", "0.0003")
KARMA.cv.debug  = CreateConVar("slc_karma_debugspam", "0")

KARMA.cv.persist = CreateConVar("slc_karma_persist", "0")
...
cvars.AddChangeCallback("slc_karma_max", function(cvar, old, new)
   SetGlobalInt("slc_karma_max", new)
end)

function KARMA.InitState()
   SetGlobalInt("slc_karma_max", config.max:GetFloat())
end
...
function KARMA.RoundIncrement()
   ...
   for _, ply in pairs(GetActivePlayers()) do
      ...
      local bonus = 0
      if frags > 0 then
         bonus = frags * config.bonus:GetInt()
      end
...
function KARMA.RoundBegin()
   ...
   if KARMA.IsEnabled() then KARMA.ApplyKarmaAll() end
end
...
function KARMA.CheckAutoKick(ply)
   if ply:GetBaseKarma() <= config.kicklevel_true:GetInt() then
