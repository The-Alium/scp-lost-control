...
function SWEP:Damage()
	...
	td.endpos = td.start + self:GetOwner():EyeAngles():Forward() * 50
	...
	if tr.Hit then
		...
		if IsValid(ent) then
			if ent:IsPlayer() or ent:IsNPC() then
				if SERVER then
					ent:TakeDamage(math.random(self.DamageMin, self.DamageMax), self:GetOwner(), self:GetOwner())
				end
...
if CLIENT then
	...
	function SWEP:PreDrawViewModel()
		...
		if vel < 10 or not self:GetOwner():OnGround() then
		...
		elseif (vel > ws * 1.2 and self:GetOwner():KeyDown(IN_SPEED)) or vel > ws * 3 then
